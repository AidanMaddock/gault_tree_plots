...
with st.sidebar:
    ...
    plots = st.multiselect("Select plots to compare:", options=df["PlotID"].unique(), max_selections = 2)
...
if uploaded_file is not None:
    ...
    if len(plots) == 2:
            ...
            for i, plot_id in enumerate(plots):
                ...
                if i == 0:
                    with col1:
                        year1 = st.selectbox("Select year to display", options=sorted(df["Year"].dropna().unique()), key = 1)
                        if year1:
                            subset = subset[subset["Year"] == year1]
                        st.subheader(f"Plot {plot_id}")
                        plot_data(subset, colors, plotting_group, year=year1)
                else:
                    with col2:
                        year2 = st.selectbox("Select year to display", options=sorted(df["Year"].dropna().unique()), key = 2)
                        if year2:
                            subset = subset[subset["Year"] == year2]
                        st.subheader(f"Plot {plot_id}")
                        plot_data(subset, colors, plotting_group, year=year2)
